...
class DatabaseConfig(BaseModel):
    ...
    url: str = Field(
        default="sqlite:///voyager_trader.db",
...
class AuditConfig(BaseModel):
    ...
    audit_all_entities: bool = Field(
        default=True,
        description="Audit all entity operations"
    )
    audit_user_actions: bool = Field(
        default=True,
        description="Audit user actions"
    )
    audit_system_events: bool = Field(
        default=True,
        description="Audit system events"
    )
    audit_trading_operations: bool = Field(
        default=True,
        description="Audit trading operations"
    )
    ...
    batch_size: int = Field(
        ...
        ge=1,
        le=1000,
        description="Batch size for audit log processing"
    )
...
class PerformanceConfig(BaseModel):
    ...
    enable_query_logging: bool = Field(
        default=False,
        description="Enable query performance logging"
    )
...
class TradingConfig(BaseModel):
    ...
    enable_pnl_calculation: bool = Field(
        default=True,
        description="Enable P&L calculation"
    )
...
class VoyagerTraderConfig(BaseModel):
    ...
    def from_env(cls) -> "VoyagerTraderConfig":
        ...
        config_data = {
            ...
            "enable_persistence": os.getenv("VOYAGER_ENABLE_PERSISTENCE", "true").lower() == "true",
            "enable_audit_logging": os.getenv("VOYAGER_ENABLE_AUDIT_LOGGING", "true").lower() == "true",
            "enable_metrics": os.getenv("VOYAGER_ENABLE_METRICS", "true").lower() == "true",
        }
        ...
        config_data["database"] = {
            ...
            "enable_wal_mode": os.getenv("DB_ENABLE_WAL_MODE", "true").lower() == "true",
            "enable_foreign_keys": os.getenv("DB_ENABLE_FOREIGN_KEYS", "true").lower() == "true",
        ...
        config_data["audit"] = {
            ...
            "audit_all_entities": os.getenv("AUDIT_ALL_ENTITIES", "true").lower() == "true",
            "audit_user_actions": os.getenv("AUDIT_USER_ACTIONS", "true").lower() == "true",
            "audit_system_events": os.getenv("AUDIT_SYSTEM_EVENTS", "true").lower() == "true",
            "audit_trading_operations": os.getenv("AUDIT_TRADING_OPERATIONS", "true").lower() == "true",
            "audit_configuration_changes": os.getenv("AUDIT_CONFIG_CHANGES", "true").lower() == "true",
        ...
        config_data["trading"] = {
            ...
            "default_order_timeout_minutes": int(os.getenv("DEFAULT_ORDER_TIMEOUT_MINUTES", "60")),
            "enable_position_tracking": os.getenv("ENABLE_POSITION_TRACKING", "true").lower() == "true",
            "enable_pnl_calculation": os.getenv("ENABLE_PNL_CALCULATION", "true").lower() == "true",
        }
