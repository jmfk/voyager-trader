...
async def test_database_transactions(temp_db):
    ...
    result = await db.execute(
        "SELECT name FROM accounts WHERE id = ?", ("test-account",), fetch="one"
    )
    ...
    result = await db.execute(
        "SELECT name FROM accounts WHERE id = ?", ("test-account-2",), fetch="one"
    )
...
async def test_database_execute_many(temp_db):
    ...
    await db.execute_many(
        "INSERT INTO accounts (id, name, account_type) VALUES (?, ?, ?)", accounts_data
    )
