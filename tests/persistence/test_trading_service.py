...
async def test_create_account_with_audit(trading_service, sample_account):
    ...
    created_account = await trading_service.create_account(
        sample_account, user_id="test-user-123"
    )
...
async def test_update_account_balances_with_audit(trading_service, sample_account):
    ...
    update_log = next(
        log for log in audit_logs if log["action"] == "account_balances_updated"
    )
...
@pytest.mark.asyncio
async def test_create_portfolio_with_audit(
    trading_service, sample_account, sample_portfolio
):
    ...
    created_portfolio = await trading_service.create_portfolio(
        sample_portfolio, user_id="test-user-123"
    )
...
async def test_create_order_with_audit(trading_service):
    ...
    created_order = await trading_service.create_order(
        order, strategy_id="momentum-strategy-1", user_id="test-user-123"
    )
    ...
    # Verify audit log
    audit_logs = await trading_service.get_audit_logs_for_entity("order", order.id)
    assert len(audit_logs) == 1
...
async def test_update_order_with_audit(trading_service):
    ...
    updated_order = created_order.execute_partial(
        quantity=Quantity(amount=Decimal("10")), price=Decimal("905.00")
    )

    # Update order
    saved_order = await trading_service.update_order(
        updated_order, strategy_id="test-strategy"
    )
...
async def test_create_trade_with_audit(trading_service):
    ...
    created_trade = await trading_service.create_trade(
        trade, strategy_id="buy-the-dip-v2"
    )
    ...
    # Verify audit log
    audit_logs = await trading_service.get_audit_logs_for_entity("trade", trade.id)
    assert len(audit_logs) == 1
...
@pytest.mark.asyncio
async def test_create_position_with_audit(
    trading_service, sample_account, sample_portfolio
):
    ...
    created_position = await trading_service.create_position(
        position, strategy_id="tech-growth-momentum"
    )
