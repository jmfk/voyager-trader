...
async def test_log_system_events(audit_service):
    ...
    shutdown_log = next(log for log in system_logs if log["action"] == "system_shutdown")
...
async def test_log_strategy_lifecycle(audit_service):
    ...
    start_log = next(log for log in strategy_logs if log["action"] == "strategy_started")
...
async def test_log_risk_events(audit_service):
    ...
    # Verify risk logs
    portfolio_logs = await audit_service.get_entity_history("portfolio", "portfolio-123")
...
async def test_log_user_sessions(audit_service):
    ...
    logout_log_id = await audit_service.log_user_logout(
        user_id=user_id,
        session_duration_minutes=125.5,
        session_id=session_id
    )
...
async def test_log_configuration_changes(audit_service):
    ...
    config_logs = await audit_service.get_entity_history("configuration", "max_position_size")
...
async def test_get_audit_trail_filtering(audit_service):
    ...
    await audit_service.log_order_submitted("order-2", "GOOGL", "sell", 50, "limit", price=2800.0)
