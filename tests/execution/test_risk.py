...
from src.voyager_trader.execution.risk import RiskManager, RiskLimits, RiskViolation
from src.voyager_trader.models.trading import Account, Order, Portfolio, Position
from src.voyager_trader.models.types import Money, OrderSide, OrderType, PositionType, Quantity, Symbol
...
class TestRiskManager:
    ...
    def test_valid_order_passes(self, risk_manager, sample_order, sample_portfolio, sample_account):
        """Test that valid order passes validation."""
        positions = []
        
        result = risk_manager.validate_order(sample_order, sample_portfolio, sample_account, positions)
        assert result is True
    
    def test_position_size_limit_violation(self, risk_manager, sample_portfolio, sample_account):
        ...
        positions = []
        result = risk_manager.validate_order(large_order, sample_portfolio, sample_account, positions)
        assert result is False
    
    def test_daily_trade_limit_violation(self, risk_manager, sample_order, sample_portfolio, sample_account):
    ...
    def test_calculate_position_size_with_stop_loss(self, risk_manager, sample_portfolio):
        ...
        expected_max_shares = Decimal("100000") * Decimal("10") / Decimal("100") / Decimal("150")
        
        assert position_size <= expected_max_shares
        assert position_size > 0
    
    def test_calculate_position_size_without_stop_loss(self, risk_manager, sample_portfolio):
        ...
        expected_shares = sample_portfolio.total_value.amount * Decimal("10") / Decimal("100") / entry_price
    ...
    def test_daily_loss_limit_shutdown(self, risk_manager):
        ...
        large_loss = Money(amount=Decimal("-1500"), currency="USD")  # Exceeds $1000 limit
    ...
    def test_margin_requirements_cash_account(self, risk_manager, sample_portfolio):
        ...
        positions = []
        result = risk_manager.validate_order(large_order, sample_portfolio, cash_account, positions)
        assert result is False
    
    def test_leverage_limit_violation(self, risk_manager, sample_portfolio, sample_account):
        ...
        result = risk_manager.validate_order(additional_order, sample_portfolio, sample_account, positions)
